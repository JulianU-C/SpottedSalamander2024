...
```{r}
# Spotted 16S PRE-process Data Analysis   ###
...
featureTab <- read.csv("spotted16S_feature_table.csv", header = T, row.names = 1, check.names=FALSE)
...
#quantsSpotted <- read.csv("quantsSpotted.csv") # qubit readings for each sample
...
meta_data <- sample_data(read.csv("spot_mdQuant.csv", header = T, row.names = 1))
...
library(Biostrings)
seqs <- readDNAStringSet("spotted16S_DNAsequences.fasta")
...
## Lost ~ 8200 taxa and ~100,000 sequences
sum(sample_sums(g16))
...
MicroStClean <- subset_samples(g16F,
                               sample_type %in% c("positive extraction control"))
...
sample_data(g16F)$is.neg <- sample_data(g16F)$sample_type %in% c("negative",
...
contamdf.combined <- isContaminant(ps.nonzero, method="combined", neg="is.neg", conc = "quant_reading", threshold = 0.25) 
...
# 2112 false, 95 true(contaminants)
...
ps.pa.neg <- prune_samples(sample_data(ps.pa)$sample_type %in% c("negative","negative extraction control","negative pcr control"), ps.pa)
# prune positive samples
ps.pa.pos <- prune_samples(sample_data(ps.pa)$sample_type %in% c("environmental","individual"), ps.pa)
...
g16F2 <- prune_taxa(!contamdf.combined$contaminant, ps.nonzero) #312 ASVs removed
...
write.csv(as(otu_table(g16F2), "matrix"), "spotted16S_feature_table_afterDecontam.csv")

library(magrittr)
## probably a good option to also run generally for any dataset after decontam
controls <- g16F2 %>%
  subset_samples(sample_type %in% c("negative",
                                    "negative extraction control",
                                    "negative pcr control", 
...
write.csv(as(otu_table(g16F3), "matrix"), "spotted16S_feature_table_afterDecontamafterExtra.csv")
...
MicroStClean2 <- subset_samples(g16F3, sample_type %in% c("positive extraction control"))
...
controls2 <- g16F2 %>%
  subset_samples(sample_type %in% c("positive pcr control", "positive extraction control",
                                   "negative pcr control", "negative", "negative extraction control")) 
...
g16F3 <- subset_samples(g16F3, !(sample_type %in% c("positive pcr control", "positive extraction control",
                                   "negative pcr control", "negative", "negative extraction control")))
...
write.csv(as(otu_table(g16F3), "matrix"), "spotted16S_feature_table_final.csv")
...
featureTab2 <- read.csv("spotted16S_feature_table_final.csv", header = T, row.names = 1, check.names = F)
...
g16F4 <- prune_samples(sample_sums(g16F4)>1200, g16F4)
...
library("ranacapa")
p <- ggrare(g16F4, step = 100, color = "location", se = FALSE)
...
## Generally, we want more like 5000 sequences per sample for amphibian skin communities to reach asymptote of curve, I'm leaning towards rarefying at 2000 - because this difference is less and 10x I'm going to do rare/un-rared analysis in parallel
...
## If correct, should all be at same sample sum, 2079
...
write.csv(alpha_df2, "spotted16S_meta_almostFinal.csv", row.names = F)
...
my_antiBd <- read.csv("spotted_blasthits_Bdstrict2020.csv", header = T, stringsAsFactors = F)
...
#write.csv(alpha_df3, "spotted16S_meta_Final.csv", row.names = F)

## This now is your clean file. All negs are removed and all samples with low coverage, anti-estimated, etc.
```


12/31/24 - Julian changed dada2 maxEE to 2,2 from 3,3
1/9/25 - Julian re-ran 2.PreProcess with new data from dada2
1/10/25 - Julian re-ran 2.PreProcess to get unrarefied anti-bd metrics
