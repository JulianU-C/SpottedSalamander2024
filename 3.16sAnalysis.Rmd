...
# Okay looks fine, only duplicate ASVs are in the shared factor, interested as to why there appear to be so many more in bar plots when earlier boxplots say environment is significantly higher richness??? - I guess there are more microbes in the environment, but each salamander is selecting a subset of those and a few unique ones, so while there is on average more variety of microbes in the environment the sals are selecting for additional ones with functional relevance??
```
...
I want to see if the ASVs in the shared group (b/w environment and sals) is
differentially abundant, if the shared ASVs are low abundance in environment and 
higher abundance in sals, this points to selection of rare taxa (I'm pretty sure)
...
Using ALDEx2, I looked for ASVs that were differentially abundant between
environmental and salamander samples. Based on a BH-corrected p-value < 0.05 and effect size > 1 or < -1, there were no ASVs that were identified as differentially abundant between the environment and the salamanders (CLR transformed, 128 monte-carlo simulations, gamma = 0.5). 
